...
<div class="row" style="padding: 20px;" >
            ...
            <hr />
                <div class="table-responsive">
                    <h4 style="width: 100%; text-align: center; font-weight: bold; line-height: 1px; padding: 10px;">LAPORAN DATA BAYAR DAFTAR ULANG</h4>
...
<thead style="width: 100%; ">
    ...
    <tr>
        ...
        <th class="text-center"><strong>UKURAN ALMAMATER</strong></th>
    </tr>
</thead>
...
<?php
    $no = 1;
    if($_POST['prodi']=='all'){
        $dataPeserta = $laporan->podb->from('peserta')->where('status_seleksi' ,'Lulus')->where('tahun_ajaran', $_POST['tahunajaran'])->where('bayar_daftarulang','Y')->orderBy('no_peserta ASC');
    }else{
        $dataPeserta = $laporan->podb->from('peserta')->where('status_seleksi' ,'Lulus')->where('pilihan_pertama', $_POST['prodi'])->where('tahun_ajaran', $_POST['tahunajaran'])->where('bayar_daftarulang','Y')->orderBy('no_peserta ASC');
    }
...
?>
    <tr>
        ...
        <td><?=strtoupper($dp['ukuran_jas']);?></td>
    </tr>
    ...
    <script src="../<?=DIR_INC;?>/js/excel/jquery.table2excel.js"></script>
    <!--
