...
<div class="row" style="padding: 20px;" >
            ...
            <hr />
                <div class="table-responsive">
                    <h4 style="width: 100%; text-align: center; font-weight: bold; line-height: 1px; padding: 10px;">LAPORAN DATA BELUM BAYAR DAFTAR ULANG</h4>
...
<?php
    $no = 1;
    if($_POST['prodi']=='all'){
        $dataPeserta = $laporan->podb->from('peserta')->where('status_seleksi' ,'Lulus')->where('tahun_ajaran', $_POST['tahunajaran'])->where('bayar_daftarulang','N')->orderBy('no_peserta ASC');
    }else{
        $dataPeserta = $laporan->podb->from('peserta')->where('status_seleksi' ,'Lulus')->where('pilihan_pertama', $_POST['prodi'])->where('tahun_ajaran', $_POST['tahunajaran'])->where('bayar_daftarulang','N')->orderBy('no_peserta ASC');
    }
