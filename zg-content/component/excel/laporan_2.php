...
<div class="row" style="padding: 20px;" >
            ...
            <hr />
                <div class="table-responsive">
                    <h4 style="width: 100%; text-align: center; font-weight: bold; line-height: 1px; padding: 10px;">LAPORAN DATA LULUS SELEKSI</h4>
...
<thead style="width: 100%; ">
    ...
    <tr>
        ...
        <th class="text-center"><strong>DAFTAR ULANG</strong></th>
    </tr>
</thead>
...
<?php
    $no = 1;
    if($_POST['prodi']=='all'){
        $dataPeserta = $laporan->podb->from('peserta')->where('status_seleksi' ,'Lulus')->where('tahun_ajaran', $_POST['tahunajaran'])->orderBy('no_peserta ASC');
    }else{
        $dataPeserta = $laporan->podb->from('peserta')->where('status_seleksi' ,'Lulus')->where('pilihan_pertama', $_POST['prodi'])->where('tahun_ajaran', $_POST['tahunajaran'])->orderBy('no_peserta ASC');
    }
    foreach($dataPeserta as $dp){
        ...
        $daftarulang = $laporan->podb->from('peserta_daftar_ulang')->where('id_peserta', $dp['id_peserta'])->fetch();
        if($daftarulang['daftar_ulang']=='Y'){
            $dftulang = 'Sudah Daftar Ulang';
        }else{
            $dftulang = 'Belum Daftar Ulang';
        }
?>
    <tr>
        <td><?=$no;?></td>
