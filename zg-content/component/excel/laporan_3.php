...
<div class="row" style="padding: 20px;" >
            ...
            <hr />
                <div class="table-responsive">
                    <h4 style="width: 100%; text-align: center; font-weight: bold; line-height: 1px; padding: 10px;">LAPORAN DATA PENDAFTAR VERIFIKASI BERKAS</h4>
...
<thead style="width: 100%; ">
    ...
    <tr>
        ...
        <th class="text-center"><strong>NIM</strong></th>
...
<?php
    $no = 1;
    if($_POST['prodi']=='all'){
        $dataPeserta = $laporan->podb->from('peserta')->where('status_seleksi' ,'Lulus')->where('verifikasi_berkas' ,'Y')->where('tahun_ajaran', $_POST['tahunajaran'])->orderBy('no_peserta ASC');
    }else{
        $dataPeserta = $laporan->podb->from('peserta')->where('status_seleksi' ,'Lulus')->where('pilihan_pertama', $_POST['prodi'])->where('verifikasi_berkas' ,'Y')->where('tahun_ajaran', $_POST['tahunajaran'])->orderBy('nim ASC');
    }
