 ...
 * This is a php file for handling admin process for registrasi page.
 *
*/
...
class registrasi extends PoCore
{
	...
	*/
	 ...
	 *
	 * This function use for index registrasi page.
	 *
	*/
	public function index()
	{
		if (!$this->auth($_SESSION['leveluser'], 'registrasi', 'read')) {
		...
		<div class="block-content">
			...
			<div class="row">
				<div class="col-md-12">
					<?=$this->pohtml->formStart(array('method' => 'post', 'action' => 'route.php?mod=registrasi&act=multidelete', 'autocomplete' => 'off'));?>
						<?=$this->pohtml->inputHidden(array('name' => 'totaldata', 'value' => '0', 'options' => 'id="totaldata"'));?>
						<?php
							$columns = array(
								array('title' => 'Id', 'options' => 'style="width:30px;"'),
								array('title' => $GLOBALS['_']['registrasi_name'], 'options' => ''),
								array('title' => $GLOBALS['_']['registrasi_email'], 'options' => ''),
								array('title' => $GLOBALS['_']['registrasi_subject'], 'options' => ''),
								array('title' => $GLOBALS['_']['registrasi_action'], 'options' => 'class="no-sort" style="width:120px;"')
							);
						?>
						<?=$this->pohtml->createTable(array('id' => 'table-registrasi', 'class' => 'table table-striped table-bordered'), $columns, $tfoot = true);?>
					<?=$this->pohtml->formEnd();?>
				</div>
			</div>
		</div>
		<?=$this->pohtml->dialogDelete('registrasi');?>
		<div id="viewdata" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true">
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-header">
						<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
						<h4 id="modal-title"><?=$GLOBALS['_']['registrasi_dialog_title_1'];?></h4>
					</div>
		...
		<div id="alertreply" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true">
			<div class="modal-dialog modal-lg">
				<div class="modal-content">
					<?=$this->pohtml->formStart(array('method' => 'post', 'action' => 'route.php?mod=registrasi&act=reply', 'autocomplete' => 'off'));?>
						<div class="modal-header">
							<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
							<h4 id="modal-title"><?=$GLOBALS['_']['registrasi_dialog_title_2'];?></h4>
						</div>
						<div class="modal-body">
							...
							<?=$this->pohtml->inputText(array('type' => 'text', 'label' => $GLOBALS['_']['registrasi_subject'], 'name' => 'subject', 'id' => 'subject', 'mandatory' => true, 'options' => 'required'));?>
							<div class="form-group">
								<label for="message"><?=$GLOBALS['_']['registrasi_message'];?> <span class="text-danger">*</span></label>
	...
	*/
	public function datatable()
	{
		...
		$columns = array(
			...
			array('db' => $primarykey, 'dt' => '5', 'field' => $primarykey,
				'formatter' => function($d, $row, $i){
					if ($row['status'] == 'Y') {
						$readdata = "<a class='btn btn-xs btn-success readdata' id='".$d."' data-toggle='tooltip' title='{$GLOBALS['_']['registrasi_notread']}'><i class='fa fa-circle-o' id='read".$d."'></i></a>";
					} else {
						$readdata = "<a class='btn btn-xs btn-success readdata' id='".$d."' data-toggle='tooltip' title='{$GLOBALS['_']['registrasi_read']}'><i class='fa fa-circle' id='read".$d."'></i></a>";
					}
					return "<div class='text-center'>\n
						<div class='btn-group btn-group-xs'>\n
							".$readdata."
							<a class='btn btn-xs btn-warning viewdata' id='".$d."' data-toggle='tooltip' title='{$GLOBALS['_']['registrasi_view']}'><i class='fa fa-eye'></i></a>
							<a class='btn btn-xs btn-primary alertreply' id='".$d."' data-name='".$row['name']."' data-email='".$row['email']."' data-subject='".$row['subject']."' data-toggle='tooltip' title='{$GLOBALS['_']['registrasi_reply']}'><i class='fa fa-reply'></i></a>
	 ...
	 *
	 * This function use for change registrasi status.
	 *
	*/
	public function readdata()
	{
		if (!$this->auth($_SESSION['leveluser'], 'registrasi', 'update')) {
			echo $this->pohtml->error();
			exit;
		}
		if (!empty($_POST)) {
			$current_registrasi = $this->podb->from('registrasi')
				->where('id_registrasi', $this->postring->valid($_POST['id'], 'sql'))
				->limit(1)
				->fetch();
			if ($current_registrasi['status'] == 'Y') {
				$status = 'N';
			} else {
				$status = 'Y';
			}
			$query_registrasi = $this->podb->update('registrasi')
	 ...
	 *
	 * This function use for display registrasi contain id.
	 *
	*/
	public function viewdata()
	{
		...
		if (!empty($_POST)) {
			$current_registrasi = $this->podb->from('registrasi')
				...
				->limit(1)
				->fetch();
			echo $current_registrasi['message'];
		}
	}
	...
	*/
	...
	{
		if (!$this->auth($_SESSION['leveluser'], 'registrasi', 'create')) {
			echo $this->pohtml->error();
			exit;
		}
		if (!empty($_POST)) {
			if ($this->posetting[23]['value'] != 'SMTP') {
				...
				$send = $poemail
					...
					->mail();
				if ($send) {
					$this->poflash->success($GLOBALS['_']['registrasi_message_1'], 'admin.php?mod=registrasi');
				} else {
					$this->poflash->error($GLOBALS['_']['registrasi_message_3'], 'admin.php?mod=registrasi');
				}
			} else {
				$this->pomail->isSMTP();
	 ...
	 *
	 * This function is used to display and process delete registrasi page.
	 *
	*/
	public function delete()
	{
		if (!$this->auth($_SESSION['leveluser'], 'registrasi', 'delete')) {
			echo $this->pohtml->error();
			exit;
		}
		if (!empty($_POST)) {
			$query = $this->podb->deleteFrom('registrasi')->where('id_registrasi', $this->postring->valid($_POST['id'], 'sql'));
			$query->execute();
			$this->poflash->success($GLOBALS['_']['registrasi_message_2'], 'admin.php?mod=registrasi');
		}
	}
	 ...
	 *
	 * This function is used to display and process multi delete registrasi page.
	 *
	*/
		...
		if (!empty($_POST)) {
			...
			if ($totaldata != "0") {
				...
				foreach($items as $item){
					$query = $this->podb->deleteFrom('registrasi')->where('id_registrasi', $this->postring->valid($item['deldata'], 'sql'));
					$query->execute();
				}
				$this->poflash->success($GLOBALS['_']['registrasi_message_2'], 'admin.php?mod=registrasi');
			} else {
				$this->poflash->error($GLOBALS['_']['registrasi_message_4'], 'admin.php?mod=registrasi');
			}
		}
	}
