 ...
 * Added in v.2.0.1
 */
$router->match('GET|POST', '/akun/profile/([a-z0-9_-]+)', function ($username) use ($core, $templates) {
 ...
 * Added in v.2.0.1
 */
$router->match('GET|POST', '/akun/profile/([a-z0-9_-]+)/page/(\d+)', function ($username, $page) use ($core, $templates) {
	$lang = $core->setlang('user', WEB_LANG);
	$count_user = $core->podb->from('users')
		->where('username', $username)
		->where('block', 'N')
 ...
 *
 */
$router->match('GET|POST', '/akun/user/edit', function () use ($core, $templates) {
 ...
 *
 */
$router->match('GET|POST', '/akun/user/changepass', function () use ($core, $templates) {
	if ($core->posetting[17]['value'] == 'Y') {
		if (empty($_SESSION['namauser_member']) and empty($_SESSION['passuser_member']) and empty($_SESSION['login_member'])) {
			header('location:' . BASE_URL . '/akun/login');
		} else {
 ...
 *
 */
$router->match('GET|POST', '/akun/user/delaccount', function () use ($core, $templates) {
	if ($core->posetting[17]['value'] == 'Y') {
		if (empty($_SESSION['namauser_member']) and empty($_SESSION['passuser_member']) and empty($_SESSION['login_member'])) {
			header('location:' . BASE_URL . '/akun/login');
		} else {
					...
					if ($validated_data === false) {
						...
						if ($user['password'] != md5($_POST['password'])) {
							...
							echo "<script language='javascript'>
								...
								window.location.href='" . BASE_URL . "/akun/logout';
							</script>";
						}
					}
				}
